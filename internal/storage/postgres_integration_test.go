...
func TestPostgresNullTimestampFieldsUseNilPointers(t *testing.T) {
	store := newTestPostgresStore(t)
	ctx := context.Background()

	if err := store.CreateTopic(ctx, &Topic{ID: "null-topic", Name: "Null Topic"}); err != nil {
		t.Fatalf("create topic: %v", err)
	}

	createdAt := time.Date(2026, 5, 1, 10, 30, 0, 123456000, time.UTC)
	if _, err := store.CreatePublishJob(ctx, &PublishJob{
		ID:        "null-job",
		TopicID:   "null-topic",
		Payload:   &model.NotificationPayload{Title: "null", Body: "shape"},
		Status:    model.NotificationJobStatusQueued,
		CreatedAt: createdAt,
	}); err != nil {
		t.Fatalf("create null timestamp job: %v", err)
	}

	job, err := store.GetJobStatus(ctx, "null-job")
	if err != nil {
		t.Fatalf("get null timestamp job: %v", err)
	}
	if !job.CreatedAt.Equal(createdAt) {
		t.Fatalf("expected created_at %s, got %s", createdAt.Format(time.RFC3339Nano), job.CreatedAt.Format(time.RFC3339Nano))
	}
	if job.ScheduledAt != nil {
		t.Fatalf("expected nil scheduled_at, got %s", job.ScheduledAt.Format(time.RFC3339Nano))
	}
	if job.CompletedAt != nil {
		t.Fatalf("expected nil completed_at, got %s", job.CompletedAt.Format(time.RFC3339Nano))
	}

	dispatchedAt := time.Date(2026, 5, 1, 10, 35, 0, 987654000, time.UTC)
	if err := store.RecordDeliveryReceipt(ctx, &model.DeliveryReceipt{
		ID:           "receipt-1",
		JobID:        "null-job",
		TokenID:      "token-1",
		...
		t.Fatalf("record typed delivery receipt: %v", err)
	}

	var storedDispatchedAt time.Time
	if err := store.db.QueryRowContext(ctx, `SELECT dispatched_at FROM delivery_receipts WHERE id = $1`, "receipt-1").Scan(&storedDispatchedAt); err != nil {
		t.Fatalf("read typed delivery receipt: %v", err)
	}
	if !storedDispatchedAt.Equal(dispatchedAt) {
		t.Fatalf("expected dispatched_at %s, got %s", dispatchedAt.Format(time.RFC3339Nano), storedDispatchedAt.Format(time.RFC3339Nano))
	}
}
